...
class DRV8833 {
  ...
  public:
    ...
    uint8_t init_pwm( uint32_t HZ, uint8_t RES ){
      ...
      PWM_MAX = ( 1 << PWM_RES ) - 1;
      PWM_HZ  = ledcAttach(0, HZ, PWM_RES);
      ledcAttach(1, PWM_HZ, PWM_RES);
      ledcAttach(2, PWM_HZ, PWM_RES);
      ledcAttach(3, PWM_HZ, PWM_RES);
    ...
    void sound_tone( uint32_t tone, uint8_t motor ){
      if( motor > 1 ) return;
      #ifdef ESP32
      ledcAttach( 0 + 2*motor , tone, PWM_RES);
      ledcAttach( 1 + 2*motor , tone, PWM_RES);
      #else
    ...
    void sound_stop( uint8_t motor ){
      if( motor > 1 ) return;
      #ifdef ESP32
      ledcAttach( 0 + 2*motor, PWM_HZ, PWM_RES);
      ledcAttach( 1 + 2*motor, PWM_HZ, PWM_RES);
      #else
      analogWriteFreq(PWM_HZ);
      #endif
